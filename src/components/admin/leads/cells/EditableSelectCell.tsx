...
export function EditableSelectCell({ value, options, onUpdate, className = '' }: EditableSelectCellProps) {
    ...
    // Apply grid navigation to this cell
    useGridNavigation(cellRef, open, () => setOpen(true));
    ...
    return (
        <div
            ...
            className="w-full flex items-center justify-start rounded outline-none focus:ring-2 focus:ring-lime focus:ring-inset focus:bg-lime/5"
        >
            <Select.Root
                value={value}
                onValueChange={(val) => {
            ...
            >
                <Select.Trigger
                    className={`group inline-flex items-center gap-2 px-1 py-0.5 text-sm text-gray-200 hover:text-white transition-colors outline-none focus:ring-0 w-full justify-between ${className}`}
                    tabIndex={-1} // remove from normal tab flow since the wrapper div handles focus
                >
                    <Select.Value>
