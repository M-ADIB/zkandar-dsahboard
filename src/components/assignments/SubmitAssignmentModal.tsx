...
import { X, FileText, Upload, CheckCircle2, Loader2, Image, Video } from 'lucide-react'
...
export function SubmitAssignmentModal({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (mode === 'file') {
            if (!file) { setError('Please select a file to upload.'); setIsSubmitting(false); return }
            ...
            if (!textValue.trim()) { setError('Please write your response.'); setIsSubmitting(false); return }
            notes = textValue.trim()
        }

        const { error: insertError } = await supabase
            .from('submissions')
            // @ts-expect-error - runtime columns added via migration
            .insert({
    ...
    const fileIsImage = file ? file.type.startsWith('image/') : false
                                ...
                                {mode === 'file' && (
                                    <div className="space-y-3">
                                        ...
                                        >
                                            {file ? (
                                                <>
                                                    {fileIsImage && <Image className="h-6 w-6 text-lime" />}
                                                    {fileIsVideo && <Video className="h-6 w-6 text-lime" />}
                                                    {!fileIsImage && !fileIsVideo && <Upload className="h-6 w-6 text-lime" />}
