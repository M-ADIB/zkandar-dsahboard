-- Trigger to sync lead updates to users table
...
DROP TRIGGER IF EXISTS tr_sync_lead_to_user ON public.leads;
CREATE TRIGGER tr_sync_lead_to_user
AFTER UPDATE ON public.leads
FOR EACH ROW
EXECUTE FUNCTION public.sync_lead_to_user();

-- Trigger to sync user updates to leads table
...
DROP TRIGGER IF EXISTS tr_sync_user_to_lead ON public.users;
CREATE TRIGGER tr_sync_user_to_lead
AFTER INSERT OR UPDATE ON public.users
FOR EACH ROW
EXECUTE FUNCTION public.sync_user_to_lead();
